...
from climahw.homework import Homework


def _compareImages(oFile, rFile):
...
class RegressionTestClass(unittest.TestCase):
    ...
    def test_regression_6(self):
        self._test_regression(
            "-s 500 500 -t 250 250 -o 125 -125 -r 0.25 --nprocs 1", "out6"
        )

    def _test_regression(self, options, oFileName):
        (rArgs, rFile) = self._mkTestCase(options, oFileName)
        self.assertTrue(_compareImages(Homework().run(rArgs), rFile))
